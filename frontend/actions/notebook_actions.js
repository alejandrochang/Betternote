...
export const fetchNotebooks = () => dispatch => (
  ...
  dispatch({type: RECEIVE_NOTEBOOKS, notebooks}), (error) =>
  dispatch({type: RECEIVE_NOTEBOOK_ERRORS, errors: error.responseJSON}))
);

export const fetchNotebook = (id) => dispatch => (
  NotebookApiUtil.fetchNotebook(id).then( ({notebook}) =>
  dispatch({ type: RECEIVE_NOTEBOOK, notebook}), (error) =>
  dispatch({type: RECEIVE_NOTEBOOK_ERRORS, errors: error.responseJSON})
  )
);

export const createNotebook = (notebookAPI) => dispatch => (
  NotebookApiUtil.createNotebook(notebookAPI).then( ({notebook}) =>
  dispatch({ type: RECEIVE_NOTEBOOK, notebook}), (error) =>
  dispatch({ type: RECEIVE_NOTEBOOK_ERRORS, errors: error.responseJSON})
  )
);
...
export const updateNotebook = (notebookAPI) => dispatch => (
  NotebookApiUtil.updateNotebook(notebookAPI).then( ({notebook}) =>
  dispatch({ type: RECEIVE_NOTEBOOK, notebook}), (error) =>
  dispatch({type: RECEIVE_NOTEBOOK_ERRORS, errors: error.responseJSON})
  )
);
...
export const deleteNotebook = (id) => dispatch => (
  NotebookApiUtil.deleteNotebook(id).then( (payload) =>
  dispatch({ type: DELETE_NOTEBOOK, payload})
  )
);
